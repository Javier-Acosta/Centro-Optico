...
3. Colecciones propuestas: `products`, `orders`, `order_items` y, si hace falta separar acceso del vendedor de `_superusers`, `seller_sessions` o una coleccion auth especifica. `products` contiene nombre, descripcion opcional, precio en unidades menores, moneda, estado publicado, estado vendido y archivo de imagen. `orders` guarda email de contacto, token publico no predecible, estado, total, moneda y snapshots.
4. Guardar importes en unidades menores enteras, moneda explicita y snapshots de los articulos en el pedido. Propuesta inicial de moneda: ARS. Despublicar o marcar como vendidos productos sin eliminar referencias historicas. Eliminar un producto borra el registro y su archivo de imagen de PocketBase; esta accion se usa cuando el vendedor quiere retirar definitivamente la publicacion.
